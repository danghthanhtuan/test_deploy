...
CREATE TABLE CONTRACTS (
    ...
    CONTRACTNUMBER VARCHAR(10) NOT NULL UNIQUE,	  --MÃ S? H?P ??NG
    STARTDATE DATETIME NOT NULL,  --NGÀY B?T ??U
    ENDDATE DATETIME NOT NULL,   -- NGÀY K?T THÚC
    SERVICE_TYPEName NVARCHAR(255) NOT NULL,
    CUSTOMERID VARCHAR(15) NOT NULL,      -- MÃ KH (S? ???C T?O T? ??NG)
    FOREIGN KEY (SERVICE_TYPEName) REFERENCES SERVICE_TYPE(SERVICE_TYPEName),
    FOREIGN KEY (CUSTOMERID) REFERENCES COMPANY(CUSTOMERID)
);
...
(
	...
	--SUPPORT NVARCHAR(40) NOT NULL,			-- LO?I H? TR?
...
CREATE TABLE PAYMENT (
    ID INT PRIMARY KEY IDENTITY(1,1) NOT NULL, 
    CONTRACTNUMBER VARCHAR(10) NOT NULL,  -- Mã yêu cầu
    CUSTOMERID VARCHAR(15) NOT NULL,  -- Mã khách hàng
    AMOUNT DECIMAL(18,2) NOT NULL,  -- Số tiền thanh toán
    PAYMENT_DATE DATETIME DEFAULT GETDATE(),  -- Ngày thanh toán
    PAYMENT_METHOD NVARCHAR(50) NOT NULL,  -- Phương thức thanh toán (Ví dụ: Chuyển khoản, tiền mặt)
    PAYMENTSTATUS NVARCHAR(20) NOT NULL,  -- Trạng thái thanh toán (VD: "Chưa thanh toán", "Đã thanh toán")
    TRANSACTION_CODE VARCHAR(50) UNIQUE NULL, -- Mã giao dịch (nếu có)
